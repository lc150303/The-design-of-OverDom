...
注：本篇默认采取任务驱动来控制creep和建筑，概念参照[hoho大佬教程](https://www.jianshu.com/p/7226e08c4b8e)与[任务驱动案例](运输模式b.md)。
### 预备知识
#### JS 函数调用
...
```
#### 能量矿产量和 Reserve Room
能量矿 source 的刷新机制是当前储量大于等于储量上线时**重置**并**关闭**计时器，当储量小于储量上限时开启计时器倒计时 300 
tick，计时归零时把储量置为储量上限。因此自然刷新速率为 **储量上限÷300t** 。   
...
## 初步
<img align="middle" src="./imgs/event1.png">
...
```
<img align="middle" src="./imgs/event2.png">

帝国能量危机得到缓解，继续派出海外矿工去过道房采集 deposit。这个 deposit 已经被我们~~迫害~~冷却时间为 52 tick，也就是采集一次后要等
51 tick 才能进行下一次采集。
```js 
// 挖 deposit 的逻辑中
...
```Game.time```来算出需要进行工作的时间，在这个时刻直接进行工作不久好啦！为了在命运中的时刻到来时触发相应的工作，我们需要把工作信息存起来，一项工作称为一个**事件**，创建新事件称为**登记**，事件的执行称为**触发**。
```js 
let 生creep的事件 = {
...
```
然后让我们像机智过人的 **[@fangxm233](https://github.com/fangxm233) 大佬**一样使用一个简单对象来存储这些所有事件记录，至于这个简单对象具体放在哪个[存储位置](存储机制.md)请根据需求自己选。
```js 
// 在初始化代码中
let 计时器 = {}
...
## 优化
类似任务机制一样，我们不希望触发事件时要使用 switch 进行多次判断，更不希望添加新事件时要修改这里的代码增加 case，
我们可以在事件信息存储对应工作的函数。对于需要参数的函数，我们把参数也存进事件信息中。
```js 
// 在管理外矿的代码中
